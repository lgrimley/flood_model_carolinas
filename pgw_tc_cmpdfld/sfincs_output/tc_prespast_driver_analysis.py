...
sbg_file = r'Z:\users\lelise\projects\ENC_CompFld\Chapter2\sfincs_models\subgrid\dep_subgrid.tif'
grid_file = r'Z:\users\lelise\projects\ENC_CompFld\Chapter2\sfincs_models\present_florence\ensmean' \
            r'\flor_ensmean_present\gis\dep.tif'
...
mod_root = [
    r'present_florence\ensmean\flor_ensmean_present',
    r'future_florence\future_florence_ensmean',
    r'future_florence\ensmean\flor_ensmean_future_45cm',
    r'present_matthew\ensmean\matt_ensmean_present',
    r'future_matthew\future_matthew_ensmean',
    r'future_matthew\ensmean\matt_ensmean_future_45cm',
]
counter = 0
fld_area_by_driver = []
...
# Reorganize the flooded area by driver as a dataframe
fld_cells = pd.DataFrame(fld_area_by_driver, columns=['no_water', 'Coastal', 'Compound-Coastal',
                                                      'Runoff', 'Compound-Runoff'])
fld_cells['Compound'] = fld_cells['Compound-Coastal'] + fld_cells['Compound-Runoff']
fld_cells.drop(['no_water', 'Compound-Coastal', 'Compound-Runoff'], axis=1, inplace=True)
fld_cells.columns = ['Coastal', 'Runoff', 'Compound']

# Calculate square km
fld_cells = fld_cells.T
fld_cells.columns = mod_root
res = 200
...
if plot_col_chart is True:
    colors = ('orange', 'plum', 'lightseagreen')
