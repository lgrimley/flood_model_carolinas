...
if load_lyrs is True:
    ...
    l_gdf = l_gdf[l_gdf['Name'].isin(
        ['Myrtle Beach', 'Wilmington', 'Raleigh', 'Charleston'])]
    l_gdf.set_index('Name', inplace=True)
    l_gdf.to_crs(epsg=32617, inplace=True)
...
# extent = np.array(basins.buffer(130000).total_bounds)[[0, 2, 1, 3]]
extent = [0.3 * (10 ** 6), 1.05 * (10 ** 6), 3.54 * (10 ** 6), 4.2 * (10 ** 6)]
''' Study Area Figure '''
plt_map = True
if plt_map is True:
    fig, ax = plt.subplots(
        ...
        tight_layout=True)
    dem = mod.grid['dep'].plot(ax=ax, cmap=cmap, norm=norm, add_colorbar=False, zorder=2)

    # Plot background/geography layers
    # basins.plot(ax=ax, color='black', edgecolor='none', linewidth=0.5, linestyle='-', zorder=0, alpha=1)
    # domain.plot(ax=ax, color='white', edgecolor='black', linewidth=0.5, zorder=1, alpha=1)
    # contour_15m.plot(ax=ax, color='lightgrey', edgecolor='none', zorder=1, alpha=1)
    # coastal_wb.plot(ax=ax, color='steelblue', edgecolor='none', linewidth=0, linestyle='-', zorder=3, alpha=1)
    states.plot(ax=ax, color='beige', edgecolor='black', linewidth=0.5, linestyle='-', zorder=0, alpha=0.5)
    ...
    st = states[states.index.isin(['North Carolina','South Carolina', 'Virginia'])]
    for label, grow in st.iterrows():
        if label == 'North Carolina':
            ann_kwargs = dict(xytext=(-130, -3), textcoords="offset points", zorder=4,
                              path_effects=[
                                  patheffects.Stroke(linewidth=2, foreground="white", alpha=1),
                                  patheffects.Normal(), ], )
        elif label == 'South Carolina':
            ann_kwargs = dict(xytext=(-60, 10), textcoords="offset points", zorder=4,
                              path_effects=[
                                  patheffects.Stroke(linewidth=2, foreground="white", alpha=1),
                                  patheffects.Normal(), ], )
        else:
...
if plt_bc_map is True:
    ...
    # Plot background/geography layers
    mod.region.plot(ax=ax, color='lightgrey', edgecolor='black', linewidth=0.5, zorder=1, alpha=0.8, label='Domain')
    states.plot(ax=ax, color='none', edgecolor='black', linewidth=0.5, linestyle='-', zorder=2, alpha=1)
    major_rivers.plot(ax=ax, color='steelblue', edgecolor='none', linewidth=0.5,
                      linestyle='-', zorder=3, alpha=1, label='Major Rivers')
    coastal_wb.plot(ax=ax, color='darkblue', edgecolor='none', linewidth=0.75,
                    linestyle='-', zorder=3, alpha=1, label='Coastal Water Bodies')
    lpd_riv.plot(ax=ax, color='steelblue', edgecolor='steelblue', linewidth=0.5, linestyle='-', zorder=3, alpha=1)
    basins.plot(ax=ax, color='none', edgecolor='black', linewidth=1,
                linestyle='-', zorder=3, alpha=1, label='HUC6 Basins')
    ...
    plt.subplots_adjust(wspace=0, hspace=0)
    plt.margins(x=0, y=0)
    plt.savefig(os.path.join(mod.root, 'gis', 'boundary_conditions.png'), bbox_inches='tight',
                dpi=255)
    plt.close()
...
if plt_bc_map is True:
    ...
    axs[0].set_title('')
    axs[0].set_title('Cumulative Precipitation', loc='left')
    axs[0].set_ylabel(f"y coordinate UTM zone {utm_zone} (m)")
    axs[0].yaxis.set_visible(True)
    axs[0].set_xlabel(f"x coordinate UTM zone {utm_zone} (m)")
    ...
    axs[1].set_title('')
    axs[1].set_title('Maximum Wind Speed', loc='left')
    axs[1].set_ylabel(f"y coordinate UTM zone {utm_zone} (m)")
    axs[1].yaxis.set_visible(True)
    axs[1].set_xlabel(f"x coordinate UTM zone {utm_zone} (m)")
