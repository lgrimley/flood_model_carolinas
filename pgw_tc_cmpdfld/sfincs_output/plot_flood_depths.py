...
type = 'mean'
da_ensmean_zsmax = xr.open_dataarray(os.path.join(work_dir, 'zsmax', f'pgw_ensmean_zsmax_{type}.nc'))

storms = ['flor', 'floy', 'matt']

ds_plot = []
for storm in storms:
    pres_zsmax = da_ensmean_zsmax.sel(run=f'{storm}_pres_compound_{type}')
    pres_hmax = (pres_zsmax - dep).compute()
    pres_hmax = pres_hmax.where(pres_hmax > 0.05)
    pres_hmax.name = f'{storm}_pres_compound_hmax'

    fut_zsmax = da_ensmean_zsmax.sel(run=f'{storm}_presScaled_compound_{type}')
    fut_hmax = (fut_zsmax - dep).compute()
    fut_hmax = fut_hmax.where(fut_hmax > 0.05)
    fut_hmax.name = f'{storm}_presScaled_compound_hmax'
...
col_title = ['Present', 'Future', 'Difference']
...
if plot_floodmap is True:
    ...
    for ax in axes:
        if counter in last_in_row:
            ...
            cs2 = ds_plot[counter].plot(ax=ax,
                                        add_colorbar=False,
                                        **ckwargs,
                                        zorder=1)
    ...
    cax = fig.add_axes([pos0.x1 + 0.02, pos0.y0 + pos0.height * -0.5, 0.025, pos0.height * 1.2])
...
for ds in ds_plot:
    ...
    df_dep_stats.to_csv(f'change_in_depth_stats_future_{type}.csv')
