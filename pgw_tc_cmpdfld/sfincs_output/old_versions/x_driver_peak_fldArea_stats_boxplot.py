...
''' FRACTIONAL AREA PLOT '''
frac_area = da_plot.copy()
frac_area['Coastal'] = frac_area['Coastal'].div(frac_area['Total'])
frac_area['Runoff'] = frac_area['Runoff'].div(frac_area['Total'])
frac_area['Compound'] = frac_area['Compound'].div(frac_area['Total'])
frac_area = frac_area.round(3)
frac_area.drop(columns='Total', inplace=True)
pres_ensmean_frac = pres_ensmean.copy()
pres_ensmean_frac['Coastal'] = pres_ensmean_frac['Coastal'].div(pres_ensmean_frac['Total'])
pres_ensmean_frac['Runoff'] = pres_ensmean_frac['Runoff'].div(pres_ensmean_frac['Total'])
pres_ensmean_frac['Compound'] = pres_ensmean_frac['Compound'].div(pres_ensmean_frac['Total'])
pres_ensmean_frac = pres_ensmean_frac.round(3)
pres_ensmean_frac.drop(columns='Total', inplace=True)

boxplot_fractional_area = False
if boxplot_fractional_area is True:
    # PLOTTING Boxplot of flooded area
    nrow = 3
    ncol = 1
    n_subplots = nrow * ncol
    ...
    for ax in axes:
        ...
        ax.scatter(x=ax.get_xticks(), y=pres_ensmean_frac[scenarios[counter]].values,
                   s=30, color='red', marker='X', zorder=2, edgecolor='black', alpha=0.9)
        if counter == 0:
    ...
    plt.subplots_adjust(wspace=0, hspace=0)
    plt.margins(x=0, y=0)
    plt.savefig('driver_fracArea_boxplot_ensmean.png', bbox_inches='tight', dpi=255)
    plt.close()

''' comparing range of wrf pres to scaled ensemble mean'''

fld_area = fld_area.drop(fld_area[fld_area['climate'] == 'pres'].index)
# Load in other runs
dir_analysis_v2 = r'Z:\users\lelise\projects\ENC_CompFld\Chapter2\sfincs_models\analysis_v2'
fld_area_v2 = pd.read_csv(
    os.path.join(dir_analysis_v2, 'driver_analysis', 'pgw_drivers_classified_all_area.csv'), index_col=0)
# remove climate change runs
cc_run_ids = pd.read_csv(os.path.join(dir_analysis_v2, 'zsmax', 'cc_run_ids.csv'), index_col=0)
cc_run_ids.columns = ['event_id']
fld_area_v2 = fld_area_v2[~fld_area_v2.index.isin(cc_run_ids['event_id'].values.tolist())]
fld_area_v2.drop(['no_flood', 'compound_coastal', 'compound_runoff'], axis=1, inplace=True)
fld_area_v2['Total'] = fld_area_v2.sum(axis=1)
scenarios = ['Coastal', 'Runoff', 'Compound', 'Total']
fld_area_v2.columns = scenarios
fld_area_v2['storm'] = [i.split("_")[0] for i in fld_area_v2.index]
fld_area_v2['climate'] = [i.split("_")[1] for i in fld_area_v2.index]
fld_area_v2['run'] = [i.split("_")[2] for i in fld_area_v2.index]
fld_area_v2['group'] = fld_area_v2['storm'] + ' ' + fld_area_v2['climate']
wrf_pres_ens = fld_area_v2[fld_area_v2['climate'] == 'pres']
wrf_pres_ens = wrf_pres_ens.drop(wrf_pres_ens[wrf_pres_ens['run'] == 'ensmean'].index)
da_plot = pd.concat([fld_area, wrf_pres_ens], axis=0)
