...
# Output shapefiles of the bbox areas for plotting
# import shapely
# extent = aoi_model.total_bounds
# polygon = shapely.geometry.box(*extent)
# gdf = gpd.GeoDataFrame(geometry=[polygon], crs=4326)
# gdf.to_file('carolinas_bbox_pgw.shp')
# x = wrf_storm_grids[2]['mslp'][0]
# polygon = x.raster.box
# gdf = gpd.GeoDataFrame(geometry=polygon['geometry'], crs=4326)
# gdf.to_file('wrf_matt_bbox_pgw.shp')
...
upper_threshold = 100
plot_thresh = 10
thresholds = np.arange(0, 15, bin_size)
...
for lower_threshold in thresholds:
    ...
    if lower_threshold == plot_thresh:
