...
building_df = pd.read_csv('SFINCS_buildings_100yr.csv', index_col=0, low_memory=True)
print(building_df.columns)

depth_thresholds = [0.05, 0.1, 0.25, 0.5, 1, 1.5, 2]
...
for depth_threshold in depth_thresholds:
    for storm in [100]:
        for clim in ['hist', 'fut']:
            ...
            flooded_pts = building_df[building_df[f'{storm}yr_{clim}_hmax'] >= depth_threshold]

            # Count the classification of the flooding (runoff, coastal, compound)
            codes, counts = np.unique(flooded_pts[f'{storm}yr_{clim}_class'], return_counts=True)

            # Save these totals to a dataframe
            d = counts.tolist() + [clim, depth_threshold]
            counts_df = pd.DataFrame(data=d, index=codes.tolist() + ['climate', 'hmin'])
            counts_df.columns = [f'{storm}yr_{clim}']
...
df = building_totals.T
df.columns = ['Coastal', 'Coastal-Comp', 'Runoff', 'Runoff-Comp', 'Period', 'hmin']
df['Compound'] = df['Coastal-Comp'] + df['Runoff-Comp']
df['Total'] = df['Compound']  + df['Coastal'] + df['Runoff']

df.to_csv('building_counts_across_depthThresh_100yr.csv')
