...
os.chdir(r'Z:\Data-Expansion\users\lelise\projects\Carolinas_SFINCS\Chapter4_Exposure\synthetic_storms')
...
# Define bin edges at 500m spacing
...
storm = 100
data_plot = []
fld_build = building_df[building_df[[f'{storm}yr_hist_zsmax', f'{storm}yr_fut_zsmax']].notna().all(axis=1)]
for clim in ['hist', 'fut']:
    # Reclass the compound
    fld_build_sub = fld_build[fld_build[f'{storm}yr_{clim}_hmax'] > depth_threshold]
    colname = f'{storm}yr_{clim}_class'
...
titles = ['Historic', 'Future', 'Future minus Historic']
nrow = 3
ncol = 1
...
plt.margins(x=0, y=0)
plt.savefig(f'building_exposure_grid_{gridsize}m.jpg', dpi=300, bbox_inches='tight')
plt.close()
