...
for depth_threshold in depth_thresholds:
    for storm in ['flor','floy','matt']:
        for clim in ['pres', 'fut']:
            ...
            flooded_pts = building_df[building_df[f'{storm}_compound_{clim}_hmax'] >= depth_threshold]

            # Count the classification of the flooding (runoff, coastal, compound)
            codes, counts = np.unique(flooded_pts[f'{storm}_{clim}_class'], return_counts=True)
...
df = building_totals.T.drop(0.0, axis=1)
...
df.to_csv('building_counts_across_depthThresh_FlorFloyMatt.csv')
